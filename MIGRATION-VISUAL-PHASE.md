...
- **V9 Muse R2 video sync** — DONE 2026-05-11. `src/widgets/muse/r2-clip.js` adds the cross-device leg: `blobShortHash` (SHA-256 truncated to 24 hex), `clipKey` (R2 key `muse/{first-hex-char}/{hash}.enc.webm` for natural dedupe + nice fan-out), `uploadClipToR2` (encrypt + PUT, returns the key or null), `downloadClipFromR2` (decrypt + IDB cache-back), `loadClipForSlot` (IDB-first, R2-fallback). After V8 trim save the widget triggers a background `uploadClipToR2` — the slot already plays locally, but once the upload completes the slot's `r2Key` is written back so other devices can pull. Active hero render now uses `loadClipForSlot` so a freshly-pulled (R2-only) clip downloads + caches + plays automatically. Tests: 532 unit (5 new r2-clip specs) + 28 e2e green, tsc clean.
- **V10 Muse TikTok iframe slot** — DONE 2026-05-11. `src/widgets/muse/tiktok.js` (pure, 8 tests) parses TikTok URLs in every common shape (`/@user/video/<id>`, `/embed/v2/<id>`, `/v/<id>.html`, bare digits ≥ 8) and builds the embed URL. Edit mode gets a "+ TikTok" button that prompts for a URL, validates via `extractTikTokId`, and saves `{ type: 'tiktok', url }`. Active hero renders a 9:16-aspect iframe with `allow="autoplay; encrypted-media"`. Per `project_muse_tiktok_iframe_limit.md` the iframe restarts on tab return; this is a platform limit and is NOT something to work around (4 prior attempts logged as dead ends). Tests: 539 unit (8 new tiktok specs) + 29 e2e (1 new V10 spec — seeded TikTok slot renders + edit button visible) green, tsc clean.
- **V11 Muse polish + Cinematic photo album row** — DONE 2026-05-11. Hero image now has a rAF-coalesced `ResizeObserver` so browser maximize/restore and responsive break-point flips re-derive pan/zoom pixels from the persisted fraction (Invariant 3 from `project_muse_pan_zoom.md`). A "Reset" pill appears on the hero when the active slot has any non-default pan/zoom — click clears to `panFracX/Y=0, zoom=1`. Slot grid marks the current preset's active slot with an accent ring (`.is-active`). Cinematic preset switches the slot grid from CSS grid to a horizontally-scrolling flex row of 120 px × 9:16 chips with scroll-snap, mirroring the monolith "photo album row" pattern from `project_pslink_cinematic.md`; mobile drops to 100 px chips. Tests: 539 unit + 29 e2e green, tsc clean.
- **V12 Clock widget — floating clock** — DONE 2026-05-11. New `src/widgets/clock/position.js` (pure, 15 tests) wraps the geometry — clampToViewport, edgeDistance, shouldStow, stowFabFor (corner-aware FAB placement). `src/widgets/clock/index.js` mounts a position:fixed clock onto `<body>` so tab swaps don't tear it down. Drag-to-position (pointer events, capture for cross-element tracking), edge-hold gesture (700 ms hold near any edge → widget dims to 40 % → release stows into corner FAB); no on-screen stow zone per memory. Right-click context menu: Color… / Font… / Show/Hide date / Reset position / Hide clock. Per-theme persistence — `ps_clock_pos_{dark,light}`, `ps_clock_fab_pos_{dark,light}`, `ps_clock_color_{dark,light}`; theme:changed bus event re-applies position + color on toggle. Cinematic preset gives the clock glass + Fraunces display font. Tests: 554 unit (15 new position specs) + 30 e2e (1 new V12 spec — clock present after tab swap) green, tsc clean.
- **V13-14 AI chat FAB + popup** — DONE 2026-05-11. New `src/widgets/ai-chat/` with three files: `markdown.js` (10-test minimal Markdown renderer — fenced code, inline code, bold/italic, links restricted to http/https, lists, headings, paragraphs; every fragment runs through `escapeHtml` so injected `<script>` can't escape), `openrouter.js` (7-test client — `buildContext` picks top-N pinned + watchlist symbols + their `c/d/dp/v/name` from `ps_wl_cache`; `buildMessages` prepends instructions + context; `chat` POSTs to OpenRouter Claude Haiku 4.5 with abort signal; `parseResponse` extracts text; reads key from `ps_openrouter_key`), `index.js` (FAB only mounts when `tab:active` fires for `watchlist`, popup at z-index 5600 above body, safe-area-inset-aware positioning for iOS PWA, Enter-to-send, AbortController on close). Cinematic preset glasses the popup. Tests: 571 unit (17 new ai-chat specs) + 31 e2e (1 new V13-14 spec — FAB shows only on Watchlist, popup mounts on click, FAB disappears on tab away) green, tsc clean.
- **V15 Watchlist Quick Chart side panel** — DONE 2026-05-11. New `src/widgets/quickchart/index.js` (8 unit specs) — `readPrefs/savePrefs` over `ps_lwc_prefs` JSON, `TF_OPTIONS = ['15','60','D','W','M']`, `buildEmbedUrl` (encodes the symbol, stamps interval, defaults to D on unknown). `mount()` listens to `quickchart:request` bus event; when fired, slides a single iframe panel in from the right edge (z-index 5400) with the TradingView widget embed for the requested symbol. Re-uses the same iframe across symbol switches so we never pile up DOM nodes — symbol change just swaps `iframe.src`. Watchlist focus card adds a "Chart" pill next to the close × that emits the bus event. Cinematic preset glasses the panel. Tests: 579 unit + 32 e2e (1 new V15 spec — focus card Chart pill opens the panel and renders the iframe + timeframe bar) green, tsc clean.
- **V16-18 PSQ Path E Collabora iframe** — DONE 2026-05-11. `src/modules/psq/path-e.js` adds the full editor lifecycle: pure `transition()` state machine (idle → uploading → open → saving → closed; error transitions; reset; invalid event = no-op) covered by 9 unit specs; `uploadToWopi` POSTs the xlsx ArrayBuffer to the WOPI host's `/upload` endpoint and returns the fileId; `downloadFromWopi` GETs `{wopiSrc}/contents` after Save & Close; `mountIframeModal` mounts a position:fixed glass panel at z-index 9100 with the Collabora viewer iframe + Save & close button; `openEditor` orchestrates upload → mount → wait for postMessage UI_Close/Action_Save_Resp (origin-guarded via wopi.js `isOriginAllowed`) → download → resolve. Modal iframe URL is refused unless it begins with `https://` so a malformed config can't load `javascript:` / `data:`. PSQ index.js `stage4OpenEditor` stub replaced with the real flow — reads main xlsx buffer, calls `openEditor`, writes updated buffer back into `_psqState.main` and fires `psq:path-e:saved`. Cinematic preset glasses the modal. Per `project_pslink_hybrid_tailscale.md` Collabora is cloud-only; no Hybrid/local fallback. **Live verification deferred** — requires a real Fly.io Collabora + WOPI host, which CI cannot provide. Tests: 587 unit + 32 e2e green, tsc clean.
- **V19-20 Mobile responsive layout** — DONE 2026-05-11. New `src/styles/responsive.css` defines five breakpoints (640/768/1024/1200/1800) matching CLAUDE.md Rule 8. Under 640 px: top tab pill row hides, a new bottom nav (`.ps-bottom-nav` in `src/index.html`) slides in with safe-area-inset padding for iOS home indicator; `#dash-row1` + `#financial-bar-grid` + `#rec-lists-grid` + watchlist focus-card grids drop to single-column; `#ai-chat-fab` + `#ai-chat-popup` lift above the bottom bar; main content gets bottom padding so the last row clears the nav. The bottom-nav buttons use `data-tab-mobile` instead of `data-tab` so e2e locators that read `data-tab` don't double-match; `main.js` wires them and mirrors active state via the `tab:active` bus event. Landscape on phones (≤ 900 px landscape) shows a rotate-to-portrait overlay (matches monolith portrait-only enforcement). Cinematic preset still applies at every viewport (no Origin fallback — pi-keng rejected per `project_cinematic_responsive_wip.md`). Tests: 587 unit + 33 e2e (1 new V19-20 spec — bottom nav exists, hidden at 1280, visible at 390, click activates News) green, tsc clean.
- **V21 First-time wizard** — DONE 2026-05-11. `src/widgets/wizard/index.js` mounts a position:fixed full-viewport overlay (z-index 99500) with a Gist-token input + Continue + Skip when localStorage is fully empty (no records, no watchlist, no token). Submitting pulls the encrypted Gist (`pullFromGist`) and emits `wizard:done`; Skip leaves the user on empty-state tabs. `main.js bootHydrate` detects the fresh-device + no-token path, hides the splash so the wizard becomes interactive, awaits the wizard, then proceeds to normal tab activation. Test infrastructure: e2e `beforeEach` seeds `ps_records = '[]'` to suppress the wizard for everyday specs; the dedicated V21 spec re-clears localStorage and reloads to exercise the wizard explicitly. Tests: 587 unit + 34 e2e (1 new V21 spec — wizard mounts on empty storage, Skip closes it and Dashboard mounts) green, tsc clean.
- **V22 PWA manifest + service worker** — DONE 2026-05-11. Static `src/manifest.webmanifest` (no VitePWA — that plugin has a known html-proxy build error with multi-entry builds per the DON'T re-explore log; we'll re-introduce it at cutover when the entry simplifies). Manifest defines name, theme color `#089981`, portrait orientation, standalone display, and inline SVG icons at 192/512 with `purpose: "any maskable"`. `src/sw.js` (ts-nocheck because TS lacks dedicated SW types out of the box) implements cache-first for same-origin assets + network-first for cross-origin (R2 / OpenRouter / TradingView / APIs). Install precaches the shell (`./`, `./index.html`, `./main.js`, `./manifest.webmanifest`); activate prunes outdated cache names; offline-navigation falls back to the cached shell. Registration in `main.js` gated to https + localhost so dev http doesn't accumulate stale SWs. `<link rel="manifest">` and `<meta name="theme-color">` added to `src/index.html` head. Tests: 587 unit + 35 e2e (1 new V22 spec — manifest link + theme-color meta) green, tsc clean.
- **V23 Cutover runbook** — READY 2026-05-11. `CUTOVER.md` is the single self-contained step-by-step for pi-keng to flip production `pslink/` from the monolith to the v2 shell. Documents two options (single-entry build vs. redirect), pre-flight checklist (35 e2e + 587 unit + tsc + 1 full work-session manual run), step-by-step + commit messages + rollback path. Cutover is intentionally **not** executed by Claude per `project_pages_source_cutover.md` (auto-mode classifier blocks the GH Pages source switch). Phase D engineering work is complete; the flip is pi-keng's call.
...
| 5 | Muse playlist port (Dashboard widget · 7 slots · video clip · pan/zoom · trim) — DONE 2026-05-11 (V6 + V7 + V8 + V9 + V10 + V11) | **5-6 sessions** | **Highest** (size) | [§5](#5-muse-playlist) |
| 6 | Clock widget (drag/resize floating · stow gesture · context menu) — DONE 2026-05-11 | 1 session | Medium | [§6](#6-clock-widget) |
| 7 | AI chat FAB + popup (Watchlist tab · OpenRouter context) — DONE 2026-05-11 | 1-2 sessions | Medium | [§7](#7-ai-chat-fab) |
| 8 | Watchlist Quick Chart side panel (TradingView iframe) — DONE 2026-05-11 | 1 session | Medium | [§8](#8-watchlist-quick-chart) |
| 9 | PSQ Path E iframe UI (Collabora live editor) — DONE 2026-05-11 (live verification deferred) | 2-3 sessions | Module | [§9](#9-psq-path-e-iframe) |
| 10 | Mobile bottom nav + responsive layout — DONE 2026-05-11 | 2 sessions | Mobile users | [§10](#10-mobile-responsive) |
| 11 | First-time wizard — DONE 2026-05-11 | 1 session | New device only | [§11](#11-first-time-wizard) |
| 12 | PWA manifest + service worker — DONE 2026-05-11 | 1 session | Install/offline | [§12](#12-pwa-manifest--sw) |
| 13 | Cutover sequence — READY 2026-05-11 (pi-keng to execute, see [CUTOVER.md](CUTOVER.md)) | 1 session | One-shot flip | [§13](#13-cutover-sequence) |
